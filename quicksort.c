...
void Quick(int vetor[10], int inicio, int fim){
   ...
   pivo = vetor[meio];
   do{
      while (vetor[i] < pivo)  i = i + 1;// vai analisar apenas valores maiores que o pivo então icrementa
      while (vetor[j] > pivo) j = j - 1;// vai analisar apenas valores menores que o pivo então icrementa
      if(i <= j){
         aux = vetor[i];
         vetor[i] = vetor[j];
         vetor[j] = aux;
